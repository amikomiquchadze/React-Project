...
const initialUsers: User[] = [
  ...
  { id: 3, name: "Charlie Brown", email: "charlie@example.com", role: "Viewer", age:"35" },
...
const UserTable: React.FC = () => {
  ...
  const openEditModal = (user: User) => {
    ...
    setFormData({ name: user.name, email: user.email, role: user.role, age: user.age });
    setShowModal(true);
  };
    ...
    <section className={styles["table-container"]}>
        ...
        <tbody>
          ...
          {users.map((user: User) => (
            <tr key={user.id}>
              ...
              <td>{user.email}</td>
              <td>{user.role}</td>
              <td> <Link to={`/counter/${user.age}`}>{user.age} </Link></td>
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{
            background: "#fff", padding: "2rem", borderRadius: "8px", width: "300px",
            ...
            <input name="role" placeholder="Role" value={formData.role} onChange={handleChange} /><br /><br />
